...
class woo_req_res_ept(models.Model):
    ...
    def req_res_data(self,method,url,verify_ssl,auth,params,data,timeout,headers,res,req_time,res_time):
        ...
        if host:
            ...
            if not woo_instance:
                woo_instance = woo_instance_obj.search([('host', '=', f"{host}/")], limit=1)
        if woo_instance and woo_instance.is_show_debug_info:
            req_type=""
            woo_req_type=False
            if url and url.split("/")[0]=="https:":
                url = f"{url}?consumer_key={params.get('consumer_key')}&consumer_secret={params.get('consumer_secret')}"
